/**
 * @preserve Basic Primitives orgDiagram v1.1.2
...
primitives.orgdiagram.Controller = function () {
	...
	this._treeItemConfigs = {};  /* primitives.orgdiagram.ItemConfig */
	this._treeItemConfigChildren = {}; /* key: primitives.orgdiagram.ItemConfig.id value: array of primitives.orgdiagram.ItemConfig.id having  ItemConfig.parent equal to key */
		...
		if ('ontouchstart' in document.documentElement) {//ignore jslint
			...
			if (newCursorItem !== null) {
				if (newCursorItem.itemConfig.id !== this.options.highlightItem) {
					...
					eventArgs.oldContext = (this._treeItemConfigs[this.options.highlightItem] != null ? this._treeItemConfigs[this.options.highlightItem].context : null);

					this.options.highlightItem = newCursorItem.itemConfig.id;

					eventArgs.context = this._treeItemConfigs[this.options.highlightItem].context;
					eventArgs.parentItem = this._highlightTreeItem.itemConfig.parent !== null ? this._treeItemConfigs[this._highlightTreeItem.itemConfig.parent] : null;
	...
	if (newCursorItem !== null) {
		if (!this._cancelMouseClick) {
			...
			if (target.hasClass(this.widgetEventPrefix + "button") || target.parent("." + this.widgetEventPrefix + "button").length > 0) {
				...
				eventArgs = new primitives.orgdiagram.EventArgs();
				eventArgs.context = newCursorItem.itemConfig.context;
				eventArgs.parentItem = newCursorItem.itemConfig.parent !== null ? this._treeItemConfigs[newCursorItem.itemConfig.parent] : null;
				...
				if (!eventArgs.cancel) {
					if (newCursorItem.itemConfig.id !== this.options.cursorItem) {
						eventArgs = new primitives.orgdiagram.EventArgs();
						eventArgs.oldContext = (this._treeItemConfigs[this.options.cursorItem] != null ? this._treeItemConfigs[this.options.cursorItem].context : null);

						this.options.cursorItem = newCursorItem.itemConfig.id;

						eventArgs.context = this._treeItemConfigs[this.options.cursorItem].context;
						eventArgs.parentItem = newCursorItem.itemConfig.parent !== null ? this._treeItemConfigs[newCursorItem.itemConfig.parent] : null;
...
primitives.orgdiagram.Controller.prototype._drawHighlight = function () {
	...
	if (this._highlightTreeItem !== null) {
		...
		uiHash = new primitives.common.RenderEventArgs();
		uiHash.context = this._highlightTreeItem.itemConfig.context;
...
primitives.orgdiagram.Controller.prototype._drawCursor = function () {
	...
	if (treeItem !== null && treeItem.actualVisibility == 1/*primitives.common.Visibility.Normal*/) {
		...
		uiHash = new primitives.common.RenderEventArgs();
		uiHash.context = treeItem.itemConfig.context;
				...
				this, function (x, y, width, height) {
					...
					switch (treeItem.actualVisibility) {
						case 1/*primitives.common.Visibility.Normal*/:
							uiHash = new primitives.common.RenderEventArgs();
							uiHash.context = itemConfig.context;
							uiHash.isCursor = treeItem.isCursor;
							uiHash.isSelected = treeItem.isSelected;
							uiHash.templateName = treeItem.template.name;
...
primitives.orgdiagram.Controller.prototype._tracePathAnnotations = function () {
    var index, len,
        index2, len2,
        index3, len3,
        firstItem, nextItem,
...
primitives.orgdiagram.Controller.prototype._findShortestPath = function (graph, startNode, endNode) {
    var margin = {},
        distance = {},
        breadcramps = {},
        marginLength = 0,
...
primitives.orgdiagram.Controller.prototype._getConnectionsGraph = function () {
...
primitives.orgdiagram.Controller.prototype._drawAnnotations = function () {
    var panel,
	...
	if (this._treeItems.hasOwnProperty(rootItemId)) {
		...
		if (segments[0].length > 0) {
		    attr = {
		        ...
		        "lineType": this.options.linesType
		    };
		    element = this.graphics.polyline(segments[0], attr);
		}

        /* draw highlighted connectors */
		if (segments[1].length > 0) {
		    attr = {
		        "borderColor": this.options.highlightLinesColor,
		        "lineWidth": this.options.highlightLinesWidth,
		        "lineType": this.options.highlightLinesType
		    };
		    element = this.graphics.polyline(segments[1], attr);
		}
	}
};
...
primitives.orgdiagram.Controller.prototype._redrawConnector = function (hasGraphics, segments, parentTreeItem, parentTreeLevel) {
    ...
    /* Find offset of horizontal connector line */
	if (parentTreeItem.partners.length > 1) {
	...
	if (parentTreeItem.visualChildren.length > 0) {
			...
			if (treeItem.connectorPlacement & 8/*primitives.common.SideFlag.Left*/ ) {
			    ...
			    this.transform.transformPoints(treeItem.actualPosition.x, treeItem.actualPosition.verticalCenter(),
                    itemToLeft.actualPosition.right(), treeItem.actualPosition.verticalCenter(), true, this, function (fromX, fromY, toX, toY) {
                        segments[treeItem.highlightPath].push(new primitives.common.MoveSegment(fromX, fromY));
                        segments[treeItem.highlightPath].push(new primitives.common.LineSegment(toX, toY));
                    });//ignore jslint
			} else if (treeItem.connectorPlacement & 2/*primitives.common.SideFlag.Right*/ ) {
			    ...
			    if (!hideConnectors) {
			        ...
			        connectorPoint = new primitives.orgdiagram.ConnectorPoint(treeItem.actualPosition.horizontalCenter(), treeItem.actualPosition.top());
			        connectorPoint.isSquared = isSquared;
			        connectorPoint.highlightPath = treeItem.highlightPath;
		...
		if (!hideConnectors && points.length > 0) {
		    hasHighlighPath = this._drawTopConnectors(segments, parentHorizontalCenter, parentTreeLevel.connectorShift, points, hasSquared);
		}

	    /* Draw connector line segment between parent and its connector line grouping children  */
		if (!hideConnectors) {
		    this.transform.transformPoints(parentHorizontalCenter, connectorOffset,
				parentHorizontalCenter, parentTreeLevel.connectorShift, true, this, function (fromX, fromY, toX, toY) {
				    segments[hasHighlighPath].push(new primitives.common.MoveSegment(fromX, fromY));
				    segments[hasHighlighPath].push(new primitives.common.LineSegment(toX, toY));
				});
		}
	}
	...
	if (parentTreeItem.partners.length > 1) {
	    ...
	    if (points.length > 0) {
	        this._drawTopConnectors(segments, parentHorizontalCenter, connectorOffset, points, true);
	    }
	}
};

primitives.orgdiagram.Controller.prototype._drawTopConnectors = function (segments, parentX, parentY, points, hasSquared) {
    ...
    if (hasSquared) {
        ...
        if (len > 0) {
            curvedPoints[len - 1].highlightPath = this._drawAngularConnectors(segments, parentX, parentY, curvedPoints, false);
            highlightPath = Math.max(highlightPath, curvedPoints[len - 1].highlightPath);
            startX = curvedPoints[len - 1].x;
        ...
        for (index = startIndex; index <= endIndex; index+= 1) {
            ...
            this.transform.transformPoints(connectorPoint.x, parentY, connectorPoint.x, connectorPoint.y, true, this, function (fromX, fromY, toX, toY) {
                segments[connectorPoint.highlightPath].push(new primitives.common.MoveSegment(fromX, fromY));
                segments[connectorPoint.highlightPath].push(new primitives.common.LineSegment(toX, toY));
                highlightPath = Math.max(highlightPath, connectorPoint.highlightPath);
            });//ignore jslint
        }
        if (startX == null) {
            startX = points[startIndex].x;
        }
        if (endX == null) {
            endX = points[endIndex].x;
        }
    } else {
        /* all lines are angular or curved */
        highlightPath = Math.max(highlightPath, this._drawAngularConnectors(segments, parentX, parentY, points, true));
        startX = endX = parentX;
    }

    /* draw common horizontal line */
    if (parentX < startX) {
        startX = parentX;
    } else if (parentX > endX) {
        endX = parentX;
    }

    if (startX !== endX) {
        if (highlightPath == 0) {
            this.transform.transformPoints(startX, parentY, endX, parentY, true, this, function (startX, startY, endX, endY) {
                segments[0].push(new primitives.common.MoveSegment(startX, startY));
                segments[0].push(new primitives.common.LineSegment(endX, endY));
            });
        } else {
            startHighlight = null;
            endHighlight = null;
            /* draw segments of highlight horizontal line */
            for (index = Math.max(0, startIndex - 1), len = Math.min(endIndex + 1, points.length - 1) ; index <= len; index += 1) {
                connectorPoint = points[index];
                if (connectorPoint.highlightPath == 1) {
                    startHighlight = (startHighlight == null) ? connectorPoint.x : Math.min(startHighlight, connectorPoint.x);
                    endHighlight = (endHighlight == null) ? connectorPoint.x : Math.max(endHighlight, connectorPoint.x);
                }
            }

            if (parentX < startHighlight) {
                startHighlight = parentX;
            } else if (parentX > endHighlight) {
                endHighlight = parentX;
            }

            if (startX < startHighlight) {
                this.transform.transformPoints(startX, parentY, startHighlight, parentY, true, this, function (startX, startY, endX, endY) {
                    segments[0].push(new primitives.common.MoveSegment(startX, startY));
                    segments[0].push(new primitives.common.LineSegment(endX, endY));
                });
            }
            if (endX > endHighlight) {
                this.transform.transformPoints(endHighlight, parentY, endX, parentY, true, this, function (startX, startY, endX, endY) {
                    segments[0].push(new primitives.common.MoveSegment(startX, startY));
                    segments[0].push(new primitives.common.LineSegment(endX, endY));
                });
            }

            this.transform.transformPoints(startHighlight, parentY, endHighlight, parentY, true, this, function (fromX, fromY, toX, toY) {
                segments[1].push(new primitives.common.MoveSegment(fromX, fromY));
                segments[1].push(new primitives.common.LineSegment(toX, toY));
            });//ignore jslint
        }
    }
...
primitives.orgdiagram.Controller.prototype._drawAngularConnectors = function (segments, parentX, parentY, points, drawToParent) {
    var joinPointX = null,
		index,
        len = points.length,
		rect,
	...
	for (index = 0; index < len; index += 1) {
	    ...
	    this.transform.transformPoint(joinPointX, parentY, true, this, function (x, y) {
	        segments[point.highlightPath].push(new primitives.common.MoveSegment(x, y));
		});//ignore jslint
		switch (this.options.connectorType) {
		    case 1/*primitives.common.ConnectorType.Angular*/:
		        this.transform.transformPoint(point.x, point.y, true, this, function (x, y) {
		            segments[point.highlightPath].push(new primitives.common.LineSegment(x, y));
		            highlightPath = Math.max(highlightPath, point.highlightPath);
		        });//ignore jslint
		        break;
		            ...
		            if (joinPointX > rect.x) {
		                this.transform.transform3Points(rect.right(), rect.verticalCenter(), rect.x, rect.verticalCenter(), rect.x, rect.bottom(), true,
							this, function (cpX1, cpY1, cpX2, cpY2, x, y) {
							    segments[point.highlightPath].push(new primitives.common.CubicArcSegment(cpX1, cpY1, cpX2, cpY2, x, y));
							    highlightPath = Math.max(highlightPath, point.highlightPath);
							});//ignore jslint
		            }
		            else {
		            ...
		            if (joinPointX > rect.x) {
		                this.transform.transformPoints(rect.x, rect.y, rect.x, rect.bottom(), true,
							this, function (cpX, cpY, x, y) {
							    segments[point.highlightPath].push(new primitives.common.QuadraticArcSegment(cpX, cpY, x, y));
							    highlightPath = Math.max(highlightPath, point.highlightPath);
							});//ignore jslint
		            } else {
...
primitives.orgdiagram.Controller.prototype._createAnnotationLabelTemplate = function () {
    var template = jQuery('<div></div>');
...
primitives.orgdiagram.Controller.prototype._onAnnotationLabelTemplateRender = function (event, data) {//ignore jslint
...
primitives.orgdiagram.Controller.prototype._readTreeItems = function () {
	...
	if (this.options.items != null && this.options.items.length > 0) {
	    ...
	    for (index = 0, len = this.options.items.length; index < len; index += 1) {
	        ...
	        if (userItem.id != null) {
	            /* override inherited ItemConfig properties with user properties */
	            itemConfig = new primitives.orgdiagram.ItemConfig();
	            for (property in itemConfig) {
	                if (itemConfig.hasOwnProperty(property)) {
	                    if (userItem.hasOwnProperty(property)) {
	                        itemConfig[property] = userItem[property];
	                    }
	                }
	            }
	            itemConfig.context = userItem;

	            this._treeItemConfigs[itemConfig.id] = itemConfig;
	            if (this._treeItemConfigChildren.hasOwnProperty(itemConfig.parent)) {
	                this._treeItemConfigChildren[itemConfig.parent].push(itemConfig);
	            } else {
	                this._treeItemConfigChildren[itemConfig.parent] = [itemConfig];
	            }

	            /* collect all items having no parent */
	            if (itemConfig.parent == null) {
	                parents.push(itemConfig);
	            }
	        }
	    }
	    ...
	    if (parents.length > 0) {
	        ...
	        for (index = 0, len = parents.length; index < len; index += 1) {
	            ...
	            switch (treeItem.itemConfig.itemType) {
	                ...
	                case 8/*primitives.orgdiagram.ItemType.AdviserPartner*/:
	                    ...
	                    break;
	                default:
	                    treeItem.actualItemType = treeItem.itemConfig.itemType;
	                    break;
	            }
	            treeItem.visibility = !treeItem.itemConfig.isVisible ? 4/*primitives.common.Visibility.Invisible*/ : 0/*primitives.common.Visibility.Auto*/;

	            this._createVisualTree(treeItem);
...
primitives.orgdiagram.Controller.prototype._getNewTreeItem = function (options) {
	...
	if (options.itemConfig != null) {
	    this._treeItemsByUserId[options.itemConfig.id] = result; /* TreeItem reference by user Id ItemConfig.id */
	}
	return result;
};
...
primitives.orgdiagram.Controller.prototype._createVisualTree = function (logicalParentItem) {
    ...
    config = logicalParentItem.itemConfig;
    ...
    treeItems = [];
    itemLogicalChildren = this._treeItemConfigChildren[logicalParentItem.itemConfig.id];
...
primitives.orgdiagram.Controller.prototype._hasVisibleChildren = function (itemConfig) {
	...
	if (!itemConfig.isVisible) {
	    result = false;
	    children = this._treeItemConfigChildren[itemConfig.id];
	    if (children != null) {
	        for (index = 0, len = children.length; index < len; index += 1) {
...
primitives.orgdiagram.Controller.prototype._hasLeavesOnly = function (treeItem) {
    ...
    if (treeItem.itemConfig !== null) {
        logicalChildren = this._treeItemConfigChildren[treeItem.itemConfig.id];
            ...
            if (len > 0) {
                ...
                for (index = 0; index < len; index += 1) {
                    ...
                    if (childItem.itemType === 0/*primitives.orgdiagram.ItemType.Regular*/) {
                        if (this._treeItemConfigChildren[childItem.id] != null) {
                            result = false;
                            break;
                        }
                    }
...
primitives.orgdiagram.Controller.prototype._readVisualTree = function (treeItem, level) {
	...
	if (itemConfig !== null) {
		...
		treeItem.actualHasSelectorCheckbox = this._getSelectionVisibility(treeItem.isCursor, itemConfig.hasSelectorCheckbox, this.options.hasSelectorCheckbox);
		treeItem.actualHasButtons = (this.options.buttons.length > 0) && this._getSelectionVisibility(treeItem.isCursor, itemConfig.hasButtons, this.options.hasButtons);
	}
...
primitives.orgdiagram.Connectors = function () {
...
primitives.orgdiagram.Connectors.prototype.toString = function () {
...
primitives.orgdiagram.TreeItem.prototype.setActualSize = function (treeLevel, options) {
	...
	switch (this.actualVisibility) {
		case 1/*primitives.common.Visibility.Normal*/:
			...
			this.actualHasGroupTitle = !common.isNullOrEmpty(this.itemConfig.groupTitle);
